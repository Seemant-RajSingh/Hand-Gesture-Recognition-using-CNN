...
#layer with 32 filters of size 3x3, using ReLU activation function and 'same' padding
classifier.add(Convolution2D(32,(3,3),input_shape=(64,64,1),activation='relu',padding='same'))
classifier.add(Convolution2D(32,(3,3),input_shape=(64,64,1),activation='relu',padding='same'))
#pool size of 3x3 and a stride of 2
...
#step3 Flatten - converts the 2D matrix data to a vector,
classifier.add(Flatten())

#creating ANN - add two fully connected layers
...
#load image path,target size of the image -  load a test image, preprocess it, and use the trained model to make predictions.
test_img= image.load_img('location of test image',target_size=(64,64),color_mode='grayscale')
test_img=image.img_to_array(test_img)
test_img=np.expand_dims(test_img,axis=0)
result= classifier.predict(test_img)
result #98.2% - clear background, 52.6% - casual background
