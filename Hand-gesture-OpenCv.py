...
# Load the hand gesture detection model
newmod = load_model('hand_gestures.h5')

# Background variables for hand segmentation
background = None
...
while True:
    ...
    if num_frames < 60:
        ...
        cv2.putText(frame_copy, "Place your hand inside the box", (330, 340), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        cv2.putText(frame_copy, "Index 0: Fist (Volume down)", (330, 355), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        ...
        cv2.putText(frame_copy, "Index 2: Straight (Volume up)", (330, 445), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        ...
        if hand is not None:
            ...
            if 0 in res:
                current_volume = max(min_volume, current_volume - volume_increment)
                pyautogui.press('volumedown')  # Adjust this if it doesn't work
            elif 6 in res:
                current_volume = min(max_volume, current_volume + volume_increment)
                pyautogui.press('volumeup')  # Adjust this if it doesn't work

            # Display the hand gesture index
            if len(res) == 0:
